package xyz.xenondevs.vetric.utils.json

import com.google.gson.*
import org.intellij.lang.annotations.Language
import xyz.xenondevs.vetric.config.ConfigSetting
import java.io.File

open class JsonConfig(val file: File, autoLoad: Boolean = true) {
    
    internal lateinit var config: JsonObject
    ...
    // Generics are not supported in enums so unchecked casting is needed.
    @Suppress("UNCHECKED_CAST")
    operator fun <T> get(setting: ConfigSetting): T? = setting.type.parse(setting.path, this) as T?
    ...
    fun <T> getOrDefault(setting: ConfigSetting, default: T): T = this[setting] ?: default
    ...
    fun <T> getOrThrow(setting: ConfigSetting, lazyMessage: () -> String = { "Missing ${setting.path} in config" }): T {
        require(setting in this, lazyMessage)
        return this[setting]!!
    }
    
    operator fun contains(path: String) = get(path) != null
    
    operator fun contains(setting: ConfigSetting) =
        setting.path in this && setting.type.isValid(setting.path, this)
